...
import { HireProfileDialog, HireProfileRequest, HireProfileRequestDisplay, HireStatusTypes } from './models';
...
export class HireRequestProfileDialogComponent extends BaseDialogComponent<HireProfileDialog> implements OnDestroy {
    ...
    hireStatusTypes = HireStatusTypes;
    ...
    onAcceptRequest(hireId, model: HireProfileRequestDisplay) {
        this.hireSub = this.hireSvc.hireRequestResponse(hireId, this.hireStatusTypes.Accepted, model).first().subscribe
            (
            r => {
    ...
    onDeniedRequest(HireID, model: HireProfileRequestDisplay) {
        this.hireSub = this.hireSvc.hireRequestResponse(HireID, this.hireStatusTypes.Denied, model).first().subscribe
            (
            r => {
                this.onSaveSuccess().then(_ => {
                    setTimeout(() => {
