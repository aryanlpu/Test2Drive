...
import { 
  ...
  ProfilePicDialogMode } from 'shared/dialogs/profile-pic/dialog.service';
...
export class ProfilePicDialog implements OnInit, OnDestroy {
  ...
  onSave(e) {
    ...
    if (this.dlgMode == ProfilePicDialogMode.ExistingProfile) {
      this.upSub = this.picSvc.uploadProfilePicture(this.data.image).subscribe(r => this.onUploaded(r));
    }
    else {
      this.onSaved.emit(this.data.image);
      this.hide();
    }
  }
