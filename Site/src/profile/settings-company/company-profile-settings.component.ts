...
import { PasswordDialogService } from 'shared/dialogs/password-dialog/dialog.service';
...
import { BudgetList } from 'profile/settings/models';
...
export class CompanyProfileSettingsComponent implements OnInit {
    ...
    constructor(
        ...
        this.passwordDialogService.activeTab$.subscribe(isActive => {
            if(isActive)
            {
    ...
    onTab(tab: string) {
        if(tab == 'payment')
        {
            this.passwordDialogService.showDialog();    
        }
        else
        {
            this.activeTab = tab;
    ...
    private setBudgetList() {
        ...
        this.lookupSvc.getBudgetKeys().forEach((item, index) => {
            ...
            if ((index > matchingIndex || this.model.defaultBudget == Budgets.InvisibleToPublic) && requirements != '') {
                ...
                this.budgetLevelList.push({ label: Budgets[item], name: Budgets[item], value: item });
            }
        });
    }
    ...
    private onSaveSuccess() {
        ...
        this.router.navigateByUrl('/profile/' + this.model.profileSysId + '/overview');
    }
    ...
    getOrganization() {
      ...
      getAllOrganizations() {
        this.profileSvc.getOrganizationDetail("company").subscribe(detail => {
