...
if [ -e "$DEPLOYMENT_TARGET/.angular-cli.json" ]; then
  cd "$DEPLOYMENT_TARGET"
  eval ./node_modules/.bin/ng set warnings.typescriptMismatch=false
...
if [ -e "$DEPLOYMENT_TARGET/web.config" ]; then
  cd "$DEPLOYMENT_TARGET"
  cp "$DEPLOYMENT_TARGET\web.config" "$DEPLOYMENT_TARGET\dist\web.config"
  exitWithMessageOnError "Error moving web.config"
  cd - > /dev/null
fi
